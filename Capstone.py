...
with tab1:
    ...
    if design_type == "Choose from Templates":
        ...
        with col1:
            st.image("https://i5.walmartimages.com/seo/Men-Heavy-Cotton-Multi-Colors-T-Shirt-Color-White-Small-Size_3cc03e24-2c97-4eaf-b99a-99eea6bb9221.1604d1a89fc6a8669bdb1896f8dd8da3.jpeg", caption="White Shirt", width=500)
        with col2:
            st.image("https://i5.walmartimages.com/seo/Men-Heavy-Cotton-Multi-Colors-T-Shirt-Color-Tweed-Small-Size_c5a4f611-2789-4081-8901-e15983d6ac9d.129328f6e3d2ab4c61120daccd4599ed.jpeg", caption="Black Shirt", width=500)
        with col3:
            st.image("https://i5.walmartimages.com/seo/Men-Heavy-Cotton-Multi-Colors-T-Shirt-Color-Neon-Blue-Small-Size_114ac5d7-632b-4975-9006-f341620eedb7.fdff6c7b97b5c4fcea877b9ce35c1598.jpeg", caption="Blue Shirt", width=500)
    ...
    if st.session_state.cart:
        ...
        for i, item in enumerate(st.session_state.cart):
            with st.container():
                ...
                with col4:
                    st.write(f"₱{item['price_per_item']}/each")
                with col5:
...
with tab2:
    ...
    with vendor_tab1:
        st.subheader("Current Pricing & Costs")
        st.write("**Current Material pricing as of September 2025:**")
        st.write(f"Shirt: ₱{mShirt}, Ink: ₱{mInk}, Packaging: ₱{mPackaging}, Total: ₱{mTotal}")
        st.write("**Labor salary as of September 2025:**")
        st.write(f"Designer: ₱{lDesigner}, Printer: ₱{lPrinter}, Packer: ₱{lPacker}, Total: ₱{lTotal}")
    ...
    with vendor_tab2:
        ...
        if st.session_state.orders:
            ...
            non_shipping_orders = [order for order in st.session_state.orders if order['status'] not in ["Shipped", "Shipping"]]
            if not non_shipping_orders:
    ...
    with vendor_tab3:
        ...
        if st.session_state.orders:
            ...
            if shipping_orders:
                for i, order in enumerate(st.session_state.orders):
                    if order['status'] in ["Shipped", "Shipping"]:
                        with st.expander(f"Order {order['order_id']} - {order['customer']} - ₱{order['total']} - Status: {order['status']}"):
                            ...
                            with col2:
                                ...
                                new_status = st.selectbox(
                                    ...
                                    key=f"shipping_status_{i}"
                                )
                                
                                if st.button("Update Status", key=f"shipping_update_{i}"):
                                    if new_status == "Completed":
                                        completed_order = {
                                            "order_id": order['order_id'],
                                            "customer": order['customer'],
